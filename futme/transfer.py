...
class TransferMarket(object):
    ...
    def price_listed_players(self, max_rating = None, normal_only=True, quick=True, sell=False):
        ...
        def match(p):
            if p['itemType'] != 'player' or p['tradeState'] is not None: return False
            if max_rating is not None and p['rating'] > max_rating: return False
            if normal_only and p['rareflag'] > 1: return False
            return True
    ...
    def search_min_price(self, player, seen_prices = 3):
        if isinstance(player, int):
            resourceId, marketMin, marketMax = player, price.MIN_PRICE, price.MAX_PRICE
        else:
            resourceId = player['resourceId']
            marketMin = player['marketDataMinPrice'] if player['marketDataMinPrice'] is not None else 150
            marketMax = player['marketDataMaxPrice'] if player['marketDataMaxPrice'] is not None else 15000000

        session = self.fme.session()
        current_player = None
        current = marketMax
        attempt = marketMax
        failed_attempt = marketMin
        seen = {}
        # XXX cap the number of iterations in case there is a bug to cause infinite loop...
        for i in range(8):
            logger.debug('round %s: current=%s, attempt=%s, failed_attempt=%s', i, current, attempt, failed_attempt)
            # if current low is just one step above hard min or last failed attempt, we are done
            if current == price.pincrement(marketMin) or current <= price.pincrement(failed_attempt):
                logger.debug('break - current is the lowest possible')
                break
            search_result = session.search('player', defId = resourceId, max_buy = attempt)
            for x in search_result:
                seen[x['id']] = x
            logger.debug('search returned %s items (%s)', 
                len(search_result), [(x['buyNowPrice'], x['expires']) for x in search_result])
            if len(search_result) == 0:
                if attempt == marketMax:
                ...
                # full page returned. set new attemp price to be between previous failed attemp and current low
                maybe_player, maybe_price = _sticky_min(search_result)
                if maybe_player is not None:
                    current_player = maybe_player
                    current = maybe_price
    ...
    def buy_now(self, resource_id, max_buy):
        ...
        while failed_bid_attempts < 3: 
            ...
            if for_sale:
                ...
                logging.info('%s items (rid=%s) for sale at %s or less', len(for_sale), resource_id, max_buy)
...
def _sticky_min(search_result):
    ...
    # sticky = [x['buyNowPrice'] for x in search_result if x['expires'] < 3540 or x['expires'] > 3600]
    # return min(sticky) if len(sticky) > 0 else None
